...
Errors.prototype = {
    ...
    explainError: function(error, colorize) {
        ...
        var location = this.getFilename() + ':' + error.line + ':' + (error.column + 1);
        result.unshift(formatErrorMessage(error.message, location, colorize));
        return result.join('\n');
    },
 ...
 * Location is filename:line:column, for example "foo/bar.js:15:37".
 *
 * @param {String} message
 ...
 * @returns {String}
 */
function formatErrorMessage(message, location, colorize) {
    return (colorize ? chalk.bold(message) : message) +
        ' at ' +
        (colorize ? chalk.green(location) : location) + ' :';
}
